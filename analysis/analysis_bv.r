...
# get the last three subjects from the old csv
results <- read.csv(
		'Pilot/results-external-pilot-bv-new-sents.csv',
		comment.char='#',
		header=FALSE, 
		quote='',
		col.names=colnames,
	...
	filter(ip_md5 %in% (ip_md5 |> unique())[4:6]) |>
...
py_run_string(
	paste0(
		...
		'CLAY Lab/structural-alternations/outputs/fhe/*bert*/bv-margs*/',
...
model.results <- model.results |>
	...
	mutate(
		condition = case_when(
						eval_data == 'syn_blorked_ext_for_human_exp' ~ 'experimental',
						TRUE ~ 'filler'
					)
	) |>
	...
	mutate(
		...
		subject = match(
			random_seed, 
			c(
				1:max(as.numeric(as.character(results$subject))), 
				random_seed |> unique()
			)
		),
...
results <- results |> 
	...
	mutate(
		...
		voice = case_when(
					voice %like% 'active' ~ 'SVO active',
					voice %like% 'passive' ~ 'OVS passive'
				),
		voice = fct_relevel(voice, 'SVO active', 'OVS passive'),
...
all.excluded.subjects <- c(
		less.than.75.on.training$subject,
		less.than.75.on.fillers$subject,
		not.above.chance.on.training.str$subject,
		purely.linear.by.arguments.both$subject
	) |> 
	sort() |>
	unique() %>%
	data.frame(subject = .) |>
...
test.data <- read.csv('accuracy-data.csv') |>
	as_tibble() |>
	filter(voice == 'OVS passive', seen_in_training == 'Seen') |>
	droplevels() |>
	mutate(
		data_source.n_i_n_target_response_a_t_subject_target = case_when(
			target_response == 'Subject target' ~ data_source.n, 
			TRUE ~ 0
		),
		data_source.n_i_n_target_response_a_t_object_target = case_when(
			target_response == 'Object target' ~ data_source.n,
			TRUE ~ 0
		)
	)
...
test.dir <- file.path('Models', 'Test')
dir.create(test.dir, showWarnings=FALSE, recursive=TRUE)

models.list <- list()

# these all work out the same
models.list['model.crossed.1'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ data_source.n * target_response.n +
		(1 + target_response.n | subject:data_source.n) +
		(1 + data_source.n | item:target_response.n),
	file = file.path(test.dir, 'model_crossed_1.rds')
))) |> list()

models.list['model.crossed.2'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ data_source.n * target_response.n +
		(1 + target_response.n | subject) +
		(1 + data_source.n | item),
	file = file.path(test.dir, 'model_crossed_2.rds')
))) |> list()

models.list['model.crossed.3'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ data_source.n * target_response.n +
		(1 + target_response.n | data_source.n:subject) +
		(1 + data_source.n | target_response.n:item),
	file = file.path(test.dir, 'model_crossed_3.rds')
))) |> list()

# nested
models.list['model.nested.1a'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target + target_response.n +
		(1 + target_response.n | subject:(data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target)) +
		(1 + data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target | item:target_response.n),
	file = file.path(test.dir, 'model_nested_1a.rds')
))) |> list()

# Model nested 1b produces error:
# 	"Error: Duplicated group-level effects not allowed."
# 	"Occured for effect 'Intercept' of group 'item:target_response.n'."
	# models.list['model.nested.1b'] <- do.call(brm, append(brm.args, list(
	# 	formula = correct ~ data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target + target_response.n +
	# 		(1 + target_response.n | subject:(data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target)) +
	# 		(1 + data_source.n_i_n_target_response_a_t_subject_target | item:target_response.n) +
	# 		(1 + data_source.n_i_n_target_response_a_t_object_target | item:target_response.n),
	# 	file = file.path(test.dir, 'model_nested_1b.rds')
	# ))) |> list()

models.list['model.nested.1c'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target + target_response.n +
		(1 + target_response.n | subject:data_source.n) +
		(1 + data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target | item:target_response.n),
	file = file.path(test.dir, 'model_nested_1c.rds')
))) |> list()

# Model nested 1d produces error:
# 	"Error: Duplicated group-level effects not allowed."
# 	"Occured for effect 'Intercept' of group 'item:target_response.n'."
	# models.list['model.nested.1d'] <- do.call(brm, append(brm.args, list(
	# 	formula = correct ~ data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target + target_response.n +
	# 		(1 + target_response.n | subject:data_source.n) +
	# 		(1 + data_source.n_i_n_target_response_a_t_subject_target | item:target_response.n) +
	# 		(1 + data_source.n_i_n_target_response_a_t_object_target | item:target_response.n),
	# 	file = file.path(test.dir, 'model_nested_1d.rds')
	# ))) |> list()

models.list['model.nested.1e'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target + target_response.n +
		(1 + target_response.n | subject:data_source.n) +
		(1 + data_source.n | item:target_response.n),
	file = file.path(test.dir, 'model_nested_1e.rds')
))) |> list()

models.list['model.nested.2a'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target + target_response.n +
		(1 + target_response.n | subject) +
		(1 + data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target | item),
	file = file.path(test.dir, 'model_nested_2a.rds')
))) |> list()

# Model nested 2b produces error:
# 	"Error: Duplicated group-level effects not allowed."
# 	"Occured for effect 'Intercept' of group 'item:target_response.n'."
	# models.list['model.nested.2b'] <- do.call(brm, append(brm.args, list(
	# 	formula = correct ~ data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target + target_response.n +
	# 		(1 + target_response.n | subject) +
	# 		(1 + data_source.n_i_n_target_response_a_t_subject_target | item) +
	# 		(1 + data_source.n_i_n_target_response_a_t_object_target | item),
	# 	file = file.path(test.dir, 'model_nested_2b.rds')
	# ))) |> list()

models.list['model.nested.2e'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target + target_response.n +
		(1 + target_response.n | subject) +
		(1 + data_source.n | item),
	file = file.path(test.dir, 'model_nested_2e.rds')
))) |> list()

models.list['model.nested.3a'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target + target_response.n +
		(1 + target_response.n | (data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target):subject) +
		(1 + data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target | target_response.n:item),
	file = file.path(test.dir, 'model_nested_3a.rds')
))) |> list()

# Model nested 3b produces error:
# 	"Error: Duplicated group-level effects not allowed."
# 	"Occured for effect 'Intercept' of group 'item:target_response.n'."
	# models.list['model.nested.3b'] <- do.call(brm, append(brm.args, list(
	# 	formula = correct ~ data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target + target_response.n +
	# 		(1 + target_response.n | (data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target):subject) +
	# 		(1 + data_source.n_i_n_target_response_a_t_subject_target | target_response.n:item) +
	# 		(1 + data_source.n_i_n_target_response_a_t_object_target | target_response.n:item),
	# 	file = file.path(test.dir, 'model_nested_3b.rds')
	# ))) |> list()

models.list['model.nested.3c'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target + target_response.n +
		(1 + target_response.n | data_source.n:subject) +
		(1 + data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target | target_response.n:item),
	file = file.path(test.dir, 'model_nested_3c.rds')
))) |> list()

# Model nested 3d produces error:
# 	"Error: Duplicated group-level effects not allowed."
# 	"Occured for effect 'Intercept' of group 'item:target_response.n'."
	# models.list['model.nested.3d'] <- do.call(brm, append(brm.args, list(
	# 	formula = correct ~ data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target + target_response.n +
	# 		(1 + target_response.n | data_source.n:subject) +
	# 		(1 + data_source.n_i_n_target_response_a_t_subject_target | target_response.n:item) + 
	# 		(1 + data_source.n_i_n_target_response_a_t_object_target | target_response.n:item),
	# 	file = file.path(test.dir, 'model_nested_3d.rds')
	# ))) |> list()

models.list['model.nested.3e'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ data_source.n_i_n_target_response_a_t_subject_target + data_source.n_i_n_target_response_a_t_object_target + target_response.n +
		(1 + target_response.n | data_source.n:subject) +
		(1 + data_source.n | target_response.n:item),
	file = file.path(test.dir, 'model_nested_3e.rds')
))) |> list()

nested.brm.args <- brm.args 
nested.brm.args['data'] <- NULL
models.list['model.pairwise.subject.1a'] <- do.call(brm, append(nested.brm.args, list(
	formula = correct ~ data_source.n +
		(1 | subject:data_source.n) +
		(1 + data_source.n | item),
	data = test.data |> filter(target_response == 'Subject target') |> droplevels(),
	file = file.path(test.dir, 'model_pairwise_subject_1a.rds')
))) |> list()

models.list['model.pairwise.subject.1b'] <- do.call(brm, append(nested.brm.args, list(
	formula = correct ~ data_source.n +
		(1 | subject) +
		(1 + data_source.n | item),
	data = test.data |> filter(target_response == 'Subject target') |> droplevels(),
	file = file.path(test.dir, 'model_pairwise_subject_1b.rds')
))) |> list()

models.list['model.pairwise.subject.1c'] <- do.call(brm, append(nested.brm.args, list(
	formula = correct ~ data_source.n +
		(1 | data_source.n:subject) +
		(1 + data_source.n | item),
	data = test.data |> filter(target_response == 'Subject target') |> droplevels(),
	file = file.path(test.dir, 'model_pairwise_subject_1c.rds')
))) |> list()

models.list['model.pairwise.object.1a'] <- do.call(brm, append(nested.brm.args, list(
	formula = correct ~ data_source.n +
		(1 | subject:data_source.n) +
		(1 + data_source.n | item),
	data = test.data |> filter(target_response == 'Object target') |> droplevels(),
	file = file.path(test.dir, 'model_pairwise_object_1a.rds')
))) |> list()

models.list['model.pairwise.object.1b'] <- do.call(brm, append(nested.brm.args, list(
	formula = correct ~ data_source.n +
		(1 | subject) +
		(1 + data_source.n | item),
	data = test.data |> filter(target_response == 'Object target') |> droplevels(),
	file = file.path(test.dir, 'model_pairwise_object_1b.rds')
))) |> list()

models.list['model.pairwise.object.1c'] <- do.call(brm, append(nested.brm.args, list(
	formula = correct ~ data_source.n +
		(1 | data_source.n:subject) +
		(1 + data_source.n | item),
	data = test.data |> filter(target_response == 'Object target') |> droplevels(),
	file = file.path(test.dir, 'model_pairwise_object_1c.rds')
))) |> list()

all.posteriors <- ldply(
		names(models.list),
		\(x) as_draws_df(models.list[[x]]) |> select(!starts_with('r_')) |> select(-lprior, -lp__) |> mutate(name=x),
		.progress='none'
	) |> 
	as_tibble() |>
...
cis <- all.posteriors |>
	...
	pivot_longer(!name, names_to='param', values_to='CI') |>
	group_by(name, param) |>
	summarize(
		across(
			everything(), 
			list(
				lower_bound = \(x) quantile(x, 0.025, na.rm=TRUE),
				median 		= \(x) quantile(x, 0.5  , na.rm=TRUE),
				upper_bound = \(x) quantile(x, 0.975, na.rm=TRUE)
			)
		)
	) |>
	arrange(name) |>
	na.omit() |>
	rowwise() |>
	mutate(label = paste0(param, ' (', str_to_title(gsub('\\.', ' ', name)), ')')) |>
	arrange(label, name) |>
	ungroup() |>
	mutate(
		name = as.factor(name) |>
			fct_relevel(rev(sort(unique(name)))),
		label = as.factor(label) |>
			fct_relevel(rev(sort(unique(label))))
	)

cis.plot <- function(df) {
	return (
		df |>
			ggplot(aes(x=CI_median, y=label)) +
			geom_point(cex=2.5) +
			geom_errorbar(aes(xmin=CI_lower_bound, xmax=CI_upper_bound)) +
			geom_vline(xintercept=0, alpha=0.5, linetype='dashed', linewidth=1.1)
	)
}

cis |>
	filter(grepl('crossed', name), grepl('^b_', label)) |>
	cis.plot() %>%
	ggsave(filename=file.path(test.dir, 'model_crossed_cis.pdf'), plot=.)

cis |>
	filter(grepl('nested', name), grepl('^b_', label)) |>
	cis.plot() %>%
	ggsave(filename=file.path(test.dir, 'model_nested_cis.pdf'), plot=.)

cis |>
	filter(grepl('pairwise', name), grepl('^b_', label)) |>
	cis.plot() %>%
	ggsave(filename=file.path(test.dir, 'model_pairwise_cis.pdf'), plot=.)

cis |>
	filter(grepl('(nested|pairwise)', name), grepl('^b_', label)) |>
	cis.plot() %>%
	ggsave(filename=file.path(test.dir, 'model_nested_pairwise_cis.pdf'), plot=.)

# END RESULT:
# models nested.1a and nested.3a perform most like the pairwise models
# the others are close enough that it doesn't make a huge difference,
# but are noticeably different regardless

########################################################################################
################### NESTED MODELS WITH INTERACTION TERM ################################
########################################################################################
library(brms)
library(beepr)
...
test.data <- read.csv('accuracy-data.csv') |>
	as_tibble() |>
	filter(seen_in_training == 'Seen') |>
	droplevels() |>
	mutate(
		target_response.n_i_n_data_source_a_t_human = case_when(
			data_source == 'human' ~ target_response.n, 
			TRUE ~ 0
		),
		target_response.n_i_n_data_source_a_t_BERT = case_when(
			data_source == 'BERT' ~ target_response.n,
			TRUE ~ 0
		)
	)
...
test.dir <- file.path('Models', 'Test-2')
dir.create(test.dir, showWarnings=FALSE, recursive=TRUE)

models.list <- list()

models.list['model.crossed.1'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ data_source.n * voice.n * target_response.n +
		(1 + target_response.n * voice.n | subject:data_source.n) +
		(1 + data_source.n | item:target_response.n:voice.n),
	file = file.path(test.dir, 'model_crossed_1.rds')
))) |> list()
beep('fanfare')
...
models.list['model.crossed.2'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ data_source.n * voice.n * target_response.n +
		(1 + target_response.n * voice.n | subject) +
		(1 + data_source.n | item),
	file = file.path(test.dir, 'model_crossed_2.rds')
))) |> list()
beep('fanfare')
...
models.list['model.crossed.3'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ data_source.n * voice.n * target_response.n +
		(1 + target_response.n * voice.n | data_source.n:subject) +
		(1 + data_source.n | target_response.n:voice.n:item),
	file = file.path(test.dir, 'model_crossed_3.rds')
))) |> list()
beep('fanfare')
...
# nested
models.list['model.nested.1a'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ (target_response.n_i_n_data_source_a_t_human + target_response.n_i_n_data_source_a_t_BERT + data_source.n) * voice.n +
		(1 + (target_response.n_i_n_data_source_a_t_human + target_response.n_i_n_data_source_a_t_BERT) * voice.n | subject:data_source.n) +
		(1 + data_source.n | item:(target_response.n_i_n_data_source_a_t_human + target_response.n_i_n_data_source_a_t_BERT):voice.n),
	file = file.path(test.dir, 'model_nested_1a.rds')
))) |> list()
beep('fanfare')
...
models.list['model.nested.1b'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ (target_response.n_i_n_data_source_a_t_human + target_response.n_i_n_data_source_a_t_BERT + data_source.n) * voice.n +
		(1 + (target_response.n_i_n_data_source_a_t_human + target_response.n_i_n_data_source_a_t_BERT) * voice.n | subject:data_source.n) +
		(1 + data_source.n | item:voice.n:(target_response.n_i_n_data_source_a_t_human + target_response.n_i_n_data_source_a_t_BERT)),
	file = file.path(test.dir, 'model_nested_1b.rds')
))) |> list()
beep('fanfare')
...
models.list['model.nested.1f'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ (target_response.n_i_n_data_source_a_t_human + target_response.n_i_n_data_source_a_t_BERT + data_source.n) * voice.n +
		(1 + target_response.n * voice.n | subject:data_source.n) +
		(1 + data_source.n | item:voice.n:target_response.n),
	file = file.path(test.dir, 'model_nested_1f.rds')
))) |> list()
beep('fanfare')

models.list['model.nested.2a'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ (target_response.n_i_n_data_source_a_t_human + target_response.n_i_n_data_source_a_t_BERT + data_source.n) * voice.n +
		(1 + (target_response.n_i_n_data_source_a_t_human + target_response.n_i_n_data_source_a_t_BERT) * voice.n | subject) +
		(1 + data_source.n | item),
	file = file.path(test.dir, 'model_nested_2a.rds')
))) |> list()
beep('fanfare')
...
models.list['model.nested.3a'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ (target_response.n_i_n_data_source_a_t_human + target_response.n_i_n_data_source_a_t_BERT + data_source.n) * voice.n +
		(1 + (target_response.n_i_n_data_source_a_t_human + target_response.n_i_n_data_source_a_t_BERT) * voice.n | data_source.n:subject) +
		(1 + data_source.n | (target_response.n_i_n_data_source_a_t_human + target_response.n_i_n_data_source_a_t_BERT):voice.n:item),
	file = file.path(test.dir, 'model_nested_3a.rds')
))) |> list()
beep('fanfare')
...
models.list['model.nested.3b'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ (target_response.n_i_n_data_source_a_t_human + target_response.n_i_n_data_source_a_t_BERT + data_source.n) * voice.n +
		(1 + (target_response.n_i_n_data_source_a_t_human + target_response.n_i_n_data_source_a_t_BERT) * voice.n | data_source.n:subject) +
		(1 + data_source.n | voice.n:(target_response.n_i_n_data_source_a_t_human + target_response.n_i_n_data_source_a_t_BERT):item),
	file = file.path(test.dir, 'model_nested_3b.rds')
))) |> list()
beep('fanfare')
...
models.list['model.nested.3d'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ (target_response.n_i_n_data_source_a_t_human + target_response.n_i_n_data_source_a_t_BERT + data_source.n) * voice.n +
		(1 + (target_response.n_i_n_data_source_a_t_human + target_response.n_i_n_data_source_a_t_BERT) * voice.n | data_source.n:subject) +
		(1 + data_source.n | voice.n:target_response.n:item),
	file = file.path(test.dir, 'model_nested_3d.rds')
))) |> list()
beep('fanfare')
...
models.list['model.nested.3f'] <- do.call(brm, append(brm.args, list(
	formula = correct ~ (target_response.n_i_n_data_source_a_t_human + target_response.n_i_n_data_source_a_t_BERT + data_source.n) * voice.n +
		(1 + target_response.n * voice.n | data_source.n:subject) +
		(1 + data_source.n | voice.n:target_response.n:item),
	file = file.path(test.dir, 'model_nested_3f.rds')
))) |> list()
beep('fanfare')

nested.brm.args <- brm.args 
nested.brm.args['data'] <- NULL
models.list['model.pairwise.human.1a'] <- do.call(brm, append(nested.brm.args, list(
	formula = correct ~ target_response.n * voice.n +
		(1 + target_response.n * voice.n | subject) +
		(1 | item:target_response.n:voice.n),
	data = test.data |> filter(data_source == 'human') |> droplevels(),
	file = file.path(test.dir, 'model_pairwise_human_1a.rds')
))) |> list()
beep('fanfare')

models.list['model.pairwise.human.1b'] <- do.call(brm, append(nested.brm.args, list(
	formula = correct ~ target_response.n * voice.n +
		(1 + target_response.n * voice.n | subject) +
		(1 | item:voice.n:target_response.n),
	data = test.data |> filter(data_source == 'human') |> droplevels(),
	file = file.path(test.dir, 'model_pairwise_human_1b.rds')
))) |> list()
beep('fanfare')

models.list['model.pairwise.human.1c'] <- do.call(brm, append(nested.brm.args, list(
	formula = correct ~ target_response.n * voice.n +
		(1 + target_response * voice.n | subject) +
		(1 | item),
	data = test.data |> filter(data_source == 'human') |> droplevels(),
	file = file.path(test.dir, 'model_pairwise_human_1c.rds')
))) |> list()
beep('fanfare')

models.list['model.pairwise.human.1d'] <- do.call(brm, append(nested.brm.args, list(
	formula = correct ~ target_response.n * voice.n +
		(1 + target_response.n * voice.n | subject) +
		(1 | target_response.n:voice.n:item),
	data = test.data |> filter(data_source == 'human') |> droplevels(),
	file = file.path(test.dir, 'model_pairwise_human_1d.rds')
))) |> list()
beep('fanfare')

models.list['model.pairwise.human.1e'] <- do.call(brm, append(nested.brm.args, list(
	formula = correct ~ target_response.n * voice.n +
		(1 + target_response.n * voice.n | subject) +
		(1 | voice.n:target_response.n:item),
	data = test.data |> filter(data_source == 'human') |> droplevels(),
	file = file.path(test.dir, 'model_pairwise_human_1e.rds')
))) |> list()
beep('fanfare')

models.list['model.pairwise.BERT.1a'] <- do.call(brm, append(nested.brm.args, list(
	formula = correct ~ target_response.n * voice.n +
		(1 + target_response.n * voice.n | subject) +
		(1 | item:target_response.n:voice.n),
	data = test.data |> filter(data_source == 'BERT') |> droplevels(),
	file = file.path(test.dir, 'model_pairwise_BERT_1a.rds')
))) |> list()
beep('fanfare')

models.list['model.pairwise.BERT.1b'] <- do.call(brm, append(nested.brm.args, list(
	formula = correct ~ target_response.n * voice.n +
		(1 + target_response.n * voice.n | subject) +
		(1 | item:voice.n:target_response.n),
	data = test.data |> filter(data_source == 'BERT') |> droplevels(),
	file = file.path(test.dir, 'model_pairwise_BERT_1b.rds')
))) |> list()
beep('fanfare')

models.list['model.pairwise.BERT.1c'] <- do.call(brm, append(nested.brm.args, list(
	formula = correct ~ target_response.n * voice.n +
		(1 + target_response * voice.n | subject) +
		(1 | item),
	data = test.data |> filter(data_source == 'BERT') |> droplevels(),
	file = file.path(test.dir, 'model_pairwise_BERT_1c.rds')
))) |> list()
beep('fanfare')

models.list['model.pairwise.BERT.1d'] <- do.call(brm, append(nested.brm.args, list(
	formula = correct ~ target_response.n * voice.n +
		(1 + target_response.n * voice.n | subject) +
		(1 | target_response.n:voice.n:item),
	data = test.data |> filter(data_source == 'BERT') |> droplevels(),
	file = file.path(test.dir, 'model_pairwise_BERT_1d.rds')
))) |> list()
beep('fanfare')

models.list['model.pairwise.BERT.1e'] <- do.call(brm, append(nested.brm.args, list(
	formula = correct ~ target_response.n * voice.n +
		(1 + target_response.n * voice.n | subject) +
		(1 | voice.n:target_response.n:item),
	data = test.data |> filter(data_source == 'BERT') |> droplevels(),
	file = file.path(test.dir, 'model_pairwise_BERT_1e.rds')
))) |> list()
beep('fanfare')

all.posteriors <- ldply(
		names(models.list),
		\(x) as_draws_df(models.list[[x]]) |> 
